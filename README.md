...
- **视频采集**：V4L2（rkisp），1920×1080 NV12，dma-buf 零拷贝
- **目标检测**：YOLOv8（RKNN，NPU 加速），人体检测 + NMS
- **硬件编码**：MPP H.264（CBR，High Profile）
- **推流**：内嵌 ZLMediaKit，支持 RTSP / HTTP-FLV
- **本地预览**：DRM + RGA 双缓冲显示，叠加检测框和实时帧率（FreeType 文字）
...
依赖（交叉编译工具链 sysroot 中已含）：MPP、RGA、libdrm、FreeType、librknnrt、ZLMediaKit(libmk_api)。

## 运行

1. 把字体文件放到板子：`/usr/share/fonts/DejaVuSansMono.ttf`
2. 把模型放到运行目录：`yolov8_person.rknn`
3. 运行：`./surveillance`
...
src/
├── main.cpp        入口
├── my_task.h/.cpp  三线程管线 + OSD
├── capture.h/.cpp  V4L2 采集
├── inference.h/.cpp RKNN 推理
├── encoder.h/.cpp  MPP 编码 + ZLMediaKit 推流
├── lcd.h/.cpp      DRM 显示
└── fps_counter.h   帧率统计
```
...
- LCD 走 RGA 90° 旋转；推流默认不旋转，如需一致请设置 `Task_config::rotation`
